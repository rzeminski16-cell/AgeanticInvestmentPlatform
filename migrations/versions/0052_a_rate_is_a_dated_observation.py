...
ADR 0078. ``aer/calc/fx.py`` has shipped complete twice — once under ADR 0026 with no
...
* The evidence is carried in two columns rather than one. ADR 0078 made
  ``source_document_id`` ``NOT NULL`` to keep out a rate with no publication behind it; ADR
  0080 moved that guarantee to ``artefact_sha256`` and let the pointer go nullable with
