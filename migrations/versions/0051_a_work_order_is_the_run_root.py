...
ADR 0068. Every run in this platform hung off a ``research_requests`` row, and so did every
...
**Three of the four steps ADR 0068 describes are in this revision**, and the fourth is
deliberately not:
...
def downgrade() -> None:
    ...
    # so the pin returns to the most recent one. That ambiguity is the cost ADR 0068 records
