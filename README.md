...
Two explainers cover the same ground from different angles, and both are worth reading
before touching the workflow:

- **`docs/anatomy-of-a-research-run.html`** — a diagram of the whole pipeline. The
  twenty-five steps in order, which are deterministic and which are model calls, where
  each gate interrupts, what all eight agents are for, and the chain from a fetched filing
  to a verified claim. Self-contained; open it in a browser.
- **`docs/notebooks/how-a-run-works.ipynb`** — the same walk-through, but every table is
  generated by importing the live code. Re-run it to find out whether the diagram has gone
  stale:
