...
and **not one of them is a stored column** (ADR 0083). There is no ``positions`` table and
...
That is the whole argument of ADR 0083 in one sentence, and it is worth restating where the
...
def pooled_cost(
    ...
    *,
    ...
    """What the shares still held cost, under ADR 0085's convention.
...
@traced(
    ...
    assumptions=(
        ...
        "before this, as its own recorded calculation over a dated rate (ADR 0082).",
...
@traced(
    ...
    assumptions=(
        "The cost is the pooled average of ADR 0085, so this is not a chargeable gain.",
...
def unrealised(_context: CalculationContext, *, value: Quantity, cost: Quantity) -> Quantity:
    ...
    **Not a gain for tax.** The cost side is a pooled average without the same-day rule, the
    thirty-day rule or share reorganisations (ADR 0085), so this answers "am I up on this?"
