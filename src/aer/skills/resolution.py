...
async def resolve_skills_for_plan(
    ...
    *before* it plans, so the planner composes its guidance from the same rows gate 1
    ...
    if existing:
        ...
        # the enabled skills or their versions changed. ADR 0072 moved pins from the plan to
...
async def pinned_skills_for_work_order(
    ...
    Pins moved from the plan to the run root in ADR 0072, which is what lets a tool with no
