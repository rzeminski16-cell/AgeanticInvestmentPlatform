...
**Nothing is stored and nothing is cached.** There is no ``positions`` table (ADR 0079), so
    ...
    *,
    ...
    Args:
        ...
        as_of: The instant the book is shown at. Positions are computed to it, prices are
            the last close on or before it, and rates likewise (ADR 0079).
    """
...
async def _current_trades(
    ...
    book — then by when it was recorded and finally by id. The pooled cost of ADR 0081
    ...
    **A genuine tie is arbitrary and stable.** Two trades of one security at the same
    ...
    matched trade, which UK rules handle with a rule ADR 0081 explicitly does not implement.
    """
...
def _acquisition_cost(context: CalculationContext, trade: Transaction) -> Quantity:
    ...
    A disposal removes cost at the pool's average rather than at what it fetched (ADR 0081),
