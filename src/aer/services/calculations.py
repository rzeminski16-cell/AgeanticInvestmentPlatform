...
class _StoredInput:
    ...
    def of(cls, raw: dict[str, Any]) -> _StoredInput:
        ...
        return cls(
            ...
            # Absent on every row written before ADR 0076. An empty string is the signal
            ...
            # evidence arithmetic that ran before it (ADR 0073).
...
async def _resolve_leaf(session: AsyncSession, stored: _StoredInput) -> LineageNode:
    ...
    A row written since ADR 0076 says which table holds it and is looked up there once. A
...
async def _fx_rate_node(session: AsyncSession, stored: _StoredInput) -> LineageNode | None:
    ...
    when this platform read that publication (ADR 0082).

    The digest is always here and the document id is not. A rate outlives the request that
    fetched it, so a purge nulls the pointer and leaves the hash (ADR 0084) — after which
...
# registry rather than two lists (ADR 0076).
#
...
# row that predates ADR 0076: nothing minted since carries an empty table.
