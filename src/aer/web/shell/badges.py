...
``adr`` field that refuses to be empty (ADR 0067).

**A badge counts rows. It never carries a figure.** ADR 0073 forbids JavaScript owning a
    ...
    for provider in providers:
        if not provider.adr.strip():
            message = (
                ...
                "a claim one tool makes on every other tool's pages, and ADR 0067 puts the "
