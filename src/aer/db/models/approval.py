...
class Approval(Base):
    ...
    # A gate hangs off the run root, not off the mandate. Until ADR 0068 this column was
    # `request_id NOT NULL`, so every approval row asserted that an equity mandate existed
    # — which made the one monitor outcome ADR 0074 preserves as a genuine human judgement
