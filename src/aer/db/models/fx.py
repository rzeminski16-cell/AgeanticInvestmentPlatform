...
somebody published, not a number in a column that gets updated. ADR 0082.
...
**The pointer to the document may be lost; the hash of what was parsed may not.** ADR 0082
made ``source_document_id`` ``NOT NULL`` to keep out a rate with no publication behind it,
and ADR 0084 moved that guarantee to ``artefact_sha256`` — because the pointer is
...
attestation under ADR 0073, carrying the weaker grade that propagates.
...
class FxRateRow(Base):
    ...
    # request needs the same row, and a published report's lineage cites it (ADR 0084).
    ...
    __table_args__ = (
        ...
        # value would be exactly the hand-typed rate ADR 0082 refused wearing a digest.
