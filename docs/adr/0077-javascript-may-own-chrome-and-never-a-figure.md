# ADR 0077 — JavaScript may own chrome and never a figure

**Status.** Accepted
**Date.** 2026-08-22
**Required by.** `docs/archive/investment-os.md` §8, which asks that the two things 0006 left implicit
...
`console.js` would without its own `@source` line. This is ADR 0071's rule one storey down: a
...
`docs/archive/investment-os.md` §8 asks for one and ADR 0081 already grounds its sample-size rule in it.
...
fact` · `Calculated` · `Attested` (ADR 0073) · `Assumed` · `Judged` (ADR 0074) — and a
