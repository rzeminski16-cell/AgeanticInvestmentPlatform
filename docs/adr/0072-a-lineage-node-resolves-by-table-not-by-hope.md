...
**Status.** Accepted
**Date.** 2026-08-22
**Required by.** `docs/investment-os.md` §6.1 — first of the four prerequisites, and the one
that note describes as a defect "written down but not yet firing" — and by ADR 0069, whose
...
which nobody will weigh as a decision because it will not look like one. And ADR 0069's
...
**ADR 0069's attestation and the macro seam are two callers of one fix.** An attestation
...
the resolver cannot find. ADR 0069 finds the same assumption in its own half of the seam —
...
0072 is where that assumption is dug out and named."
...
to alter, so adding a value costs one line — which is exactly how ADR 0069 prices the arrival
