...
ADR 0074 decides that a thesis monitor's `contradicted` status is the one monitor outcome
...
The one monitor outcome 0074 is careful to preserve as a genuine human judgement is the one
outcome the schema forbids recording.

That is this record's problem rather than 0074's. **This ADR owns the run root, and a gate
...
**ADR 0074's tiering decision presumes this migration.** Four statuses becoming findings and
...
**The argument for it is made in ADR 0074 and settled here**, because this is already the
migration that moves the run root and a correlation vocabulary is a property of that root.
0074's case is that a trade entry, a position correction, a thesis edit and an
