# ADR 0071 — A tool is a registered capability, not a package

**Status.** Accepted
**Date.** 2026-08-22
**Required by.** `docs/archive/investment-os.md` §7. ADRs 0072 to 0081 each describe a capability
...
Those are ADR 0072's problem. This record decides only how a capability is admitted, and
...
kind.** ADR 0072 gives a run root a `(subject_kind, subject_id)` pair with no foreign key
...
read (ADR 0076). A declared kind with no registered resolver is refused at import, in the
...
`docs/developers/knowledge-map.md` §7 lists five ways to extend this platform and says the sixth way
...
root (ADR 0072), its own gates, its own tables. That is why it is settled by a registry row
...
`docs/developers/knowledge-map.md`. It would change the `mypy` strict globs, `aer.core.*` and
...
boundary from `docs/developers/knowledge-map.md` and a test rather than from `ls`.
...
**The registry refuses a tool with no ADR**, so each of ADRs 0072 to 0081 is a precondition
...
line of `docs/developers/knowledge-map.md` still resolves. Migration is lazy and per-module, driven by
what a second tool actually needs — starting with the run root, which is ADR 0072's.
