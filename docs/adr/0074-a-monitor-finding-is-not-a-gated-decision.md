...
`docs/PLAN.md` §2.4 — low source coverage, potential look-ahead, validation failure and
...
contradicted, unobservable}` (ADR 0075). Four of those describe evidence moving. One
...
rows the platform can hold. ADR 0077 scores the process rather than the outcome, and the
