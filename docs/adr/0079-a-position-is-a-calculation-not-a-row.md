...
**Extends.** ADR 0069, which admitted the attestation as the fourth record class, and ADR
...
`transactions`, as an attestation subtype under ADR 0069. One row per thing that happened
...
and a settlement date, a quantity, a price, a currency, fees, and — from ADR 0069 — a
**grade**:
...
- **market value** — quantity times a `price_bars` close for that security on or before
  the as-of date, converted through a dated rate (ADR 0078)
...
ADR 0069's containment applies here without amendment. A position computed from any
...
A portfolio is continuous and a research run is a point (ADR 0071). This record does not
...
kind — which is what ADR 0069 promised when it argued for the fourth.
