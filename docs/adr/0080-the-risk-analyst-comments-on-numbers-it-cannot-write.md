# ADR 0080 — The risk analyst comments on numbers it cannot write

**Status.** Accepted
**Date.** 2026-08-22
**Required by.** ADR 0035, which holds that a new agent role needs an ADR before it needs
code, and `docs/archive/investment-os.md` §9, which calls this one "barely a model role".
...
on ADR 0082's rate store and ADR 0045's pivot arithmetic, not on anything a model could supply —
...
**`conviction` is deliberately not on that list, and it does not wait for sizing.** ADR 0074
...
than the role — worth learning before ADRs 0079 and 0081 commit to the same shape.
...
equity research mandate behind it, and that generalisation is ADR 0072's. And containing the
