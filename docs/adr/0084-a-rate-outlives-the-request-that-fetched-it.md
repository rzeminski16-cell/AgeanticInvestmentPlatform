# ADR 0084 — A rate outlives the request that fetched it

**Status.** Accepted
**Date.** 2026-08-23
**Amends.** ADR 0082, in one column. Everything else that record decided stands: the table,
...
**Required by.** The purge. ADR 0082 was written before its column met
`services/requests.py`, and the two disagree.

## Context

ADR 0082 gave `fx_rates.source_document_id` a `NOT NULL` and an `ON DELETE RESTRICT`,
...
**This keeps ADR 0082's guarantee more completely than the column it replaces.** That record
...
remember, and does not degrade. The door ADR 0082 was closing stays closed, and stays closed
afterwards.
...
**One decision in ADR 0082 is superseded and the rest is not.** The `NOT NULL` on the
