...
ADR 0069 admits an `attested` Attestation — a value backed by nothing but the operator's word
...
disappear: ADR 0069's propagation is a return type with no field for the figure, so a NAV three
...
me those bytes" does not — the property `price_bars` already relies on. ADR 0071 raised this
for the marks; it applies unchanged to the rates those marks were translated with. And
`fx_rates` is another leaf table for ADR 0072's resolver registry, arriving after that
