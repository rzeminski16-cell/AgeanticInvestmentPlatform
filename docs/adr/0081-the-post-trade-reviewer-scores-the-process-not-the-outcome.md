# ADR 0081 — The post-trade reviewer scores the process, not the outcome
...
The platform has refused this once already, from the other side. ADR 0079 denies the thesis
...
`untested`, `unobservable`. `unobservable` carries ADR 0079's meaning and 0079's argument:
...
enough to state.** `docs/archive/investment-os.md` §10 records what prose composition still needs
...
between this role and ADR 0079's. Comparing what was expected against what happened — the
...
change a holding cannot bias one — the property the monitor lacks, and the reason 0079's ban
...
thesis. It is noise with a P&L attached.** ADR 0079 argued that in full and this record does
not repeat it.
...
and the rate store ADR 0082 decides — which is a reason for it to be
...
**A lesson is a Judgement (ADR 0074), with exactly the privileges that record grants.** It
...
**The argument is not that the taxonomy is closed, because it is not.** ADR 0073 opens it:
...
evidence behind it. That is the standard 0073 set — a kind is admitted because it brings a
story, never as an exemption from having one — and a lesson meets no part of it. So the
refusal here cannot rest on a count of enum members, and does not: ADR 0074 grants a
...
but the platform's own row. Whether this tightens the loop ADR 0079 describes is a fair
...
takes the shape ADR 0077 gives the provenance badge, where `ProvenanceRef(kind, identifier,
...
to a dozen outcomes and written by the system that graded them — which is ADR 0079's objection
