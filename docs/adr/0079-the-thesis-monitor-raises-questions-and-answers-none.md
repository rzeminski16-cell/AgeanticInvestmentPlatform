# ADR 0079 — The thesis monitor raises questions and answers none
...
Where the output lands is ADR 0078's — `contradicted` opens a gate, the other four statuses
...
0081's, which scores the process rather than the outcome — and it never touches thesis
status.
...
**`contradicted` opens a gate, and nothing else updates anything** (ADR 0078). 0064's real
...
answer is a decision a person takes, and the record of it is a Judgement (ADR 0074), which
this role cannot write. Nor does it score what happened afterwards — that is ADR 0081's, and
...
bought at one model call per premise per evidence event against the monthly cap ADR 0078
