...
*The orientation layer. Read this first, before `docs/PLAN.md` (the authority on scope),
...
- **The Investment OS expansion** — the platform growing from one research tool into several,
  ...
  source, not a number in a column), 0079 (a position is a calculation, not a row), 0080 (a
  rate outlives the request that fetched it), 0081 (cost basis is a pooled average, and not
  a tax computation).
...
3. The module docstrings of `calc/units.py`, `workflow/engine.py`, `providers/anthropic.py`
   and `agents/registry.py` — the four densest statements of intent in the codebase.
4. `docs/gap-analysis.md` — the honest distance between plan and reality, including what
   testing methods found what.
5. `docs/PLAN.md` — only then, and as a reference rather than a read-through.

The test suite is the sixth document: `just test` (unit, no network, no spend),
