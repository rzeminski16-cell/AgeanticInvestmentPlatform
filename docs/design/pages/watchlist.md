...
| | |
|---|---|
| **URLs** | `/watchlist` · `POST /watchlist` · `POST /watchlist/{entry_id}/commission` · `POST /watchlist/{entry_id}/stop` · `POST /watchlist/commission-next` |
...
(with how many runs at the cap it holds), and the cost guidance the request form shows
(what runs at the standard depth have cost, or an honest admission of no history).
...
and the report. A status chip. **Commission** with an *As at* date defaulting to today
(absent while a run is alive), and **Stop following because** with a required reason.
Beneath the list, **Commission the next the budget affords**, disabled with the reason when
the budget is spent.
...
**Withdrawn entries are not shown.** They are kept with their reason and reachable from
nothing yet.
...
**1. A withdrawn list**, as the decisions journal has, so the record of what was once
followed is a page rather than a table.

**2. The cost guidance beside the button** rather than in the budget sheet, so *Commission
the next* says what it is about to spend.

**3. A "researched as at" history per entry**, listing every commission rather than the
latest, once an entry has several.
...
* Stopping following with a reason takes the row away.
