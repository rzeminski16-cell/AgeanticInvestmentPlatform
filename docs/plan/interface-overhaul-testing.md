...
| **1 — Vocabulary** | **Labels and tones done.** Every member of eight mapped enums has a label and a tone; a new one without either is a red build, watched failing. A guardrail and a fault can never share a tone. Cost renders identically everywhere. **Still owed:** every composed verdict rendering from a thin fixture as well as a rich one | `tests/test_presentation_vocabulary.py`, `tests/test_web_figures.py`, `tests/test_verdict.py` |
