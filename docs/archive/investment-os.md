...
design note, not a plan: `docs/archive/PLAN.md` remains the authority on scope, and §3 below is a
...
## 3. Where this sits against `docs/archive/PLAN.md` — a question, not an answer
...
invalidate file references across 66 ADRs and `docs/developers/knowledge-map.md`, change the
...
`conviction` is due **now**, with ADR 0074 rather than with any sizing work: a skill that can
...
`weight`, `recommended_weight`, `action`, `order_quantity`, `stop_loss` — are ADR 0080's, and
...
```text
0071  a tool is a registered capability, not a package
0072  a work order is the run root and a research request is a detail of one
0073  an attestation is what the book says, at two times and one grade of evidence
0074  a judgement is never a source reference
0075  the portfolio clock is not the research clock
0076  a lineage node resolves by table, not by hope        (repairs the macro seam)
0077  javascript may own chrome and never a figure          (amends 0006)
0078  a monitor finding is not a gated decision
0079  the thesis monitor raises questions and answers none
0080  the risk analyst comments on numbers it cannot write
0081  the post-trade reviewer scores the process, not the outcome
0082  a rate is a dated observation with a source, not a number in a column
```

0082 was not in the first draft of this list. It was added because three of the others name
...
portfolio tables. Settled with the operator on 23 August, recorded in ADR 0083:

11. `fx_rates` and `services/fx.py`; `aer.calc.fx` gets its first caller (ADR 0082).
12. `attestations` and `transactions`; the grade that propagates (ADR 0073).
13. `aer/calc/portfolio.py` — quantity, cost basis, market value, cash, NAV, weight, each a
    recorded calculation. **No `positions` table** (ADR 0083).
...
1. **A thesis item is a note that may carry a predicate.** The statement is free text; the
   ...
   well" has no metric. Price is not the test: outcome testing is ADR 0081's, and it never
   touches thesis status. Decided in ADR 0079.
...
4. **`approvals.gate` becomes a `tool_gates` reference table** with a real composite foreign
   ...
   truth that can only fail at INSERT. Decided in ADR 0071 — and this is the one decided
...
2. **May the knowledge graph become a JS island?** ADR 0077 admits islands by name and
...
3. **What does a lapsed market-data subscription do to NAV history?** ADR 0075 decides it;
...
- [x] ADRs 0071–0081 drafted
- [x] ADRs 0071–0081 repaired — nine blocking issues from the first review, then five more
      from the second, including two records that still claimed the macro defect was firing
- [x] ADR 0082 — a rate is a dated observation with a source. Added during review, because
      three of the others named an FX rate store as a prerequisite and none could decide it
- [x] `knowledge-map.md` §6 cites 0082, now that the record exists
...
- [x] Operator sign-off: ADR 0075's answer on a lapsed subscription accepted — bars go,
...
- [x] Polymorphic lineage resolver — a source reference carries a table discriminator
      (ADR 0076). `SourceRef.fact()` is gone; one constructor per relation. Two relations
...
- [x] `work_orders` supertype; `research_requests` demoted to a 1:1 detail row sharing its
      key; `jobs` gains `work_order_id` (ADR 0072). Migration 0051 — steps 1–3 of four,
      with a working downgrade proved by the round-trip test
- [x] `approvals.request_id` repointed to `work_orders` — ADR 0078's gate cannot be written
      until this lands
...
- [x] `plan_skill_pins` repointed off `research_plans`, unblocking skills platform-wide
      ...
      These six landed in one commit because ADR 0072 says splitting them would be worse
...
### Two things the migration surfaced that ADR 0072 did not anticipate
...
- [x] **A dry run needs its own work order.** Pins are unique per (work order, skill), and
      ...
      job, step and spend, so under ADR 0072's own definition it is its own unit of work
...
- [x] Provenance badge requires a `ProvenanceRef`; two chips, not five (ADR 0077). A ref
...
- [x] `drawer.js` — focus trap, Escape, scroll lock, `aria-modal`. Written once, and
      ...
      **It opens because content arrived, not because anything told it to.** There is no
      ...
      owns. A grep asserts no page grows a second focus trap (ADR 0077), and the trap
...
- [ ] Guidance mode — the flag, the route and `data-guidance` on `<body>` are done: server
      state under ADR 0077, a form POST that redirects so it works with scripting off, and a
...
- [x] Badge counts off the critical render path, behind `GET /_shell/badges`. A
      ...
      **The swap is `hx-swap-oob="innerHTML"`, not the default**, and that is the whole
      accessibility question ADR 0077 named: `"true"` replaces the element, so the
...
- [x] **ADR 0083 accepted**, 23 August. A position is a calculation, not a row
...
- [x] **One registry for tools, three states.** `WORKING` earns a navigation section,
      ...
      planned. This is ADR 0071's `ToolDefinition` in the only shape that is useful yet;
...
- [x] **The sidebar became a dropdown.** `<details>`/`<summary>` and **no JavaScript at
      ...
      which ADR 0077 spends a paragraph refusing, and it would be dead with scripting off.
...
- [x] `fx_rates` and `services/fx.py`; `aer.calc.fx` gets its first caller (ADR 0082)
- [x] ADR 0084 — a rate outlives the request that fetched it. Written during the slice, not
      before it: ADR 0082's `NOT NULL` source document met `purge_request` and made a
...
- [x] `attestations` and `transactions`, with the grade that propagates (ADR 0073), and
...
- [x] **Macro is still a seam and this did not close it.** A `macro_observations` row is
      ...
      only wrapped in a calculation. ADR 0073 named the question and picked neither answer;
...
- [x] `aer/calc/portfolio.py` — quantity, cost basis, market value, cash, NAV, weight. Pure,
      `mypy --strict`, property-tested. **No `positions` table** (ADR 0083)
- [x] ADR 0085 — cost basis is a pooled average, and not a tax computation. ADR 0083 named
      ...
      than a rewrite
- [x] ADR 0085's worked example is in the golden corpus as well as the unit tests: it is the
...
- [ ] Judgements and theses (ADRs 0074, 0079)
- [ ] The thesis monitor (ADRs 0078, 0079)
- [ ] Portfolio risk and scenarios (ADR 0080)
- [ ] Trade journal, post-trade review and decision analytics (ADR 0081)
- [ ] Watchlist and Research Queue
- [ ] `RESERVED_OUTPUT_FIELDS` gains `conviction` (ADR 0074), with its attack file, and the
