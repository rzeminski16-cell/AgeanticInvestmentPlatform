...
| ~~A23~~ | ~~**Two of the three call paths composed their own user turn**~~ | **Closed.** A leftover from A14, in the place A14 did not look. `Agent.run` was taught to send `stable_context` as a separate cached block; `run_batch` and `estimate_input_tokens` each built `Message(role="user", …)` by hand and predate it, so both omitted the prefix. For the estimator that is an under-count of the largest part of a call. For the batch path it is worse than a miscount — the prompt actually sent has its evidence **missing**, and the input-cap check under-counts to match. Inert today only because the one role with a stable context does not go through the batch path, which is not a property anything enforced. All three now share `Agent.compose_turn`, and a test parses `base.py` and asserts a user turn is constructed in exactly one place, because the real invariant is "there is one constructor" rather than "these three happen to agree". |
