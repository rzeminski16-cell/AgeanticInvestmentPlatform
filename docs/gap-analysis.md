...
| # | Gap | Notes |
|---|---|---|
| ~~A16~~ | ~~**`tests/test_extraction.py` deadlocks in a full-suite run**~~ | **Closed, and it was worse than recorded here — ADR 0047.** Not pytest's threads: numpy's bundled OpenBLAS, pulled in transitively by `arelle`, starts a worker thread per core, and with those present the child watcher's `Thread.start()` never returns — `/proc` shows the waitpid thread was never created. It wedges *before* `_run_child` arms its parse timeout, so the timeout cannot fire; the worker carries the same threads, so this was a production hazard, not a test-ordering nuisance. A prefix bisect took 41 files down to a two-file reproduction; pinning the BLAS pool to one thread in `aer/__init__.py` fixes it (three green runs against two hangs). The mechanism is not fully root-caused — a standalone reproduction of churn-plus-fork-plus-OpenBLAS completes every time — so this is an honest workaround, held by `tests/test_blas_threads.py`. |
