...
| B3 | **Comparables never appear** | No longer blocked on A20 — the licence question is settled and the containment is built. What remains is the same assumption question as B2: the multiples that need a forecast need confirmed assumptions. Since the 2026-08-09 determination a comps table may appear in an exported report; the underlying price chart may not. |
