...
|---|---|
| Microsoft's own P/E of **27.43×** and EV/EBITDA of **18.93×** — computed, traced and replayed on every run | ~~`render/document.py:436` types the comps parameter `WithheldComps \| None`, so no multiple can pass through it. The docstring says why: a licence position the operator **reversed on 2026-08-09** (`fetch/policy.py:192`, `derived_figures_publishable=True`)~~ **Closed by §4.3 on 17 September 2026.** The signature now takes what `for_audience` returns, and the row was right that the licence was not the obstacle — but it named one wall where there were three. The second: the render path hand-built the withheld form rather than reading the table back, so there was no route for a figure even once the type allowed it. The third: the step recorded the figure without the calculation behind it, so no footnote could have resolved (§3.19.8) |
...
| A conclusion to reach | ~~`vertical_slice_v1.py:3765` assigns `report.rating = None`. Nothing in the codebase ever writes it, so "no view reached" is printed unconditionally — a hardcoded `None`, not a judgement~~ **Closed by §4.4 on 18 September 2026**, and not by writing the column. `reports.rating` is still `None` and should be: ADR 0117 puts a rating in the *authored* half, stored as a judgement with a holder, a time, a basis and a falsifier, because a column has none of those. What changed is the header line and the block under it — a base-case range by method, the distance from the price, and the sensitivity swing, every figure a recorded calculation with a resolving marker. "No view reached" survives for a run with no valuation, where it is true. *Corrected 25 September 2026 (ADR 0132):* the range in the masthead was read by every verdict-round judge as a view the document then contradicted, so the header now says what each method gives, the block says why the two differ, and the view line reads "none stated" until the operator states one — "no view reached" is printed nowhere |
...
| # | What | Effort |
|---|---|---|
| 3.1 | **Port F-27's calculation index into `gather_evidence`** — order by name then newest period, one row per `(name, case)`, skip sensitivity cells, carry the period, bound the pool. The query is already written, at `services/red_team.py:343-374` | **Done 17 September 2026, £0.** The rule moved to `services/calculations.py` and both callers read it, so the adversary and the sections can no longer disagree about what the run computed. One decision the port forced: a section's bound is its token budget rather than the cap, so the cut would fall in name order and take `value_per_share` and `wacc` first, alphabetically last as they are — run-level figures therefore sort *first* for a section. The comment that argued the opposite was right about its own world and is rewritten rather than deleted: it is the sensitivity grid that made nulls-first unsafe, and the grid is now excluded. The full-run golden moved by one figure, which is the fake writer taking the pack's first row |
| 3.2 | **Cross-section consistency** over the assembled draft: no figure name+period carrying two values; no sentence asserting a figure is unavailable that another section prints | **Done 17 September 2026, £0, under ADR 0125.** `services/consistency.py` now makes three passes rather than one: published facts down the ladder as before, published calculations, and a denial scan over the draft's own sentences. Advisory — it records escalated `disagreements` rows and refuses nothing; the promotion is a separate decision, routed through the final gate's evidence rule and never through `eval/metrics.py`, after two consecutive clean live runs. Measured twice ([the readout](../plan/readiness-audit-2026-09/cross-section-check-dry-run.md)): offline against the committed exports, **one contradiction across seven**; then against the live re-seeded corpus, where the first shipped version recorded **eighteen** — the offline run read the rendered Markdown against a whole-ledger index where the code reads section content against the published-figure index. Four narrowings took it **18 → 5 → 2 → 1**, each with a regression test: the platform's own record sections are not scanned, a narrowed denial is a different subject, a clause quoting the figure is using it, and the negation reaches forwards and stops at the word about the record. What survives is one denial per four runs, and it is real — AZN #2 denying an interest cover it computes, footnotes and quotes at 8.11× in three other sections. That exercise found two defects in the check before it shipped: `Calculation.name` is the *function's* name, so `days_outstanding` serves three ratios at once; and a discount factor takes its year as a parameter, not an input. Both are fixed by keying on the question a calculation answers — its `calc.engine` identity minus its output |
| 3.3 | **The adversary stops lying, and the appendix stops lying about the adversary.** Re-check every challenge against the calculation record before publication; rewrite the appendix after settlement | **The appendix half is done, 17 September 2026, £0** (ADR 0115 decision 4, now Accepted in part). The settle path refills the disagreements section before it re-seals, so the report says what was settled, by whom in effect and why, instead of printing "Escalated for human decision at approval" over every challenge whatever became of it. The rule is spoken rather than named and the winner is its label rather than "position A". **The re-check half is withdrawn as specified and re-specified in the ADR**, because measuring it against the run it was argued from refuted its premise: all **33 of 33** numerals in AZN #2's three false challenges are real recorded values. They quote FY2025 *and* FY2021 of one figure side by side and call the pair a contradiction — the very comparison `services/consistency.py` forbids the platform's own document. The cause is already closed structurally by 3.1's period-labelled index; the residual rule worth building is named in ADR 0115 and waits to be measured first |
...
| # | What | Notes |
|---|---|---|
| 4.1 | **Fix the price layer** (`services/price_acquisition.py`, the bars count that loses the whole layer on a warm database) | **Done 17 September 2026, £0.** One line of meaning: `_Listing.bars` was `stored.inserted`, and `record_bars` inserts only what is new — so the *second* acquisition of a company inserted nothing, already held everything, and was told "the market-data provider returned no prices". It now counts what the platform **holds** (inserted + already held + in conflict), and keeps the insert count beside it because "what changed" is still a real question. AZN #2 and M&T were both second runs. Two smaller things rode with it: `record_bars` read a narrower window than it wrote, so it could insert a row it had never compared — harmless today because the adapter bounds the response, and a `uq_price_bars_day` violation on a re-run the day it is not; and the price stub returned bars past the as-of date that the real adapter drops, which is how an unfaithful stub hides a bug and then produces a crash of its own |
| 4.2 | **Prefer market equity in the WACC** | **Done 17 September 2026, £0.** The calc layer was already ready — `EquityBasis.MARKET` existed and `cost_of_capital` attached the caveat only for `BOOK`. What was missing was the wiring, under a module docstring reading *"Nothing in this workflow acquires a price"* that had outlived the price step by months. `_value` now passes the market capitalisation the price step computes, exactly as the comps step already did. Two states still take book and each says which: no price at all, and a price in a currency the filed balance sheet is not in — the second gets its own caveat, because one naming the wrong reason sends a reader to fix the wrong thing. **And the valuation had no declared dependency on the price step**: both descend from `gate_unmapped_concepts` and neither waited for the other, so the basis would have depended on which finished first. A WACC market-weighted on a fast day and book-weighted on a slow one is worse than one always book, because nothing in the report says which happened |
| 4.3 | **Carry the subject's own multiples into the document** — §17 prints 27.4× and 18.9× with footnotes resolving to the recorded calculations | **Done 17 September 2026, £0, under ADR 0034's amendment.** First, a correction to §1's diagnosis above: the sentence every audited run actually printed is not the withholding paragraph but its sibling — "every one of the eight proposed peers was excluded … **No comparable figure was computed**, and there is no fuller version elsewhere" — printed over the subject's own multiples, of which every one of the eight committed run exports holds at least one and MSFT #2 holds four (roadmap §3.19.11). Then three walls, and the licence was not one of them. ADR 0030's amendment of 2026-08-09 had already decided this — it says in as many words that "the comps section of an exported report now shows the multiples" — and ADR 0034 had already named its own price: "one method and one signature". The method changed in August; the signature did not, so for six weeks the determination was encoded everywhere except the place that decides what a reader sees. The second wall was the route: the render path hand-built a `WithheldComps` from two integers on the step's record while the valuation page read the row lists beside them, so nothing held the two readings together; both now go through `comps_table_from_record` and ask `for_audience`. The third was provenance — **the step recorded the figure and not the calculation behind it**, so a footnote could only have named the step, and a marker resolving to nothing prints the report's own broken-citation notice against a sound figure. The id is on the quantity already, because `@traced` puts it there; recording it cost one field. The rule that follows is the one this pays for: **a figure prints only with a marker that resolves**, so a record written before the id was stored renders exactly as it did before — the disclosure, and no table |
| 4.4 | **An implied upside/downside** against the price, and the composed base-case range in the header instead of "no view reached" | **Done 18 September 2026, £0, ADR 0117 Accepted in part.** Not one render branch, and not `rating`. An upside is a *number*, so it is a traced calculation struck by the valuation step — a renderer subtracting a price from a value would produce a figure no ledger row accounts for and no footnote could point at. And the rating column is the **authored** half's, which ADR 0117 stores as a judgement with a holder, a time, a basis and a falsifier precisely because a column has none of those; `reports.rating` stays unwritten and the header line is what changes. Built as ADR 0117's composed half entire — the base case by method, the distance from the price for *both* terminal methods (never their average, ADR 0038), the sensitivity swing in words, and the range in the masthead — placed first in the document, because a position a reader meets after eighteen sections is one they meet last. Three things the build found: the price step computed a price and threw it away, so a run with no mappable share count held none either; the market capitalisation's recorded source was a literal string that could never resolve; and no run in the corpus has ever had a scenario, so the spread ADR 0117 describes is a category the block supports and nothing has yet filled |
...
**Exit, and it is two exits** (corrected 17 September 2026; roadmap §3.19.9). Re-render MSFT #1
and AZN #2 from stored rows — an implied range, segment figures and resolvable footnotes, with
no sentence denying any of them. That proves everything this phase changes about *how a record
is drawn*. It cannot prove 4.1, 4.2 or 4.3, each of which changes what a **step records**: a
second run's bar count, the capital structure the WACC weighs, the calculation id beside a
multiple. A run seeded before those changes does not hold the rows, and re-rendering one says
so — correctly, and to no one's benefit. Those three are proved on a run made after the change:
the offline full-run fixture for £0, and Phase 7's verdict round on real filings.
