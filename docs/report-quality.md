...
| **R13** | **Interest expense does not resolve, and it takes the whole valuation with it** | **The cause of the missing DCF, and the highest-priority finding in this file.** `_cost_of_debt` derives the pre-tax cost of debt as interest expense over average debt. When `_line(latest, "interest_expense")` returns nothing and the balance sheet carries debt, it raises `ValuationNotPossibleError` — and the run loses the discount rate, the forecast, the scenario bridge, the sensitivity grid, the football field and the valuation section. For CHRW that happened with **eleven of eleven assumptions confirmed and every driver present**. One unresolved line defeated everything else. Two things are wrong and they are separable. **First, the concept map probably does not carry CHRW's spelling**: `US_GAAP_ALIASES` maps `InterestExpense` and `InterestExpenseNonoperating`, while a company presenting "interest and other expense, net" may tag `InterestIncomeExpenseNonoperatingNet`, `InterestExpenseDebt` or similar. The run swept **181 unmapped tags**; the answer is in that list, and the fix is the discipline A62 used — read the filer's actual tag, add it by name, guess nothing from its shape. **Second, and structurally worse: the cost of debt is the one discounted-cash-flow input a person cannot supply.** `PROPOSABLE_NAMES` is `REQUIRED_NAMES` plus per-year drivers, and `cost_of_debt` appears in neither. Every other input to the valuation — including the three the platform refuses to derive — can be typed at the gate and confirmed. This one is derived-or-nothing, so an operator who hits it has no move at all: no form, no override, and a method note that explains the refusal without telling them it is unfixable from where they are standing. Fix, in order: (a) find the tag and map it; (b) add `cost_of_debt` to the proposable set so a missing line is an assumption a person can make, on the record, with their name against it — which is precisely what the platform already does for the risk-free rate; (c) have the assumptions gate name this dependency *before* approval, since today the operator only learns of it after the run finishes.
...
**First — the cost of debt (R13).** Not a readability finding at all: it is why three
consecutive runs have shipped without a valuation, and on this run it discarded eleven
confirmed assumptions over one unresolved line. Map the tag, then make `cost_of_debt`
suppliable by hand so that no single absent fact can defeat a forecast the operator has
otherwise fully specified. Everything else in this file makes the report read better;
this makes it contain the thing it exists for.
