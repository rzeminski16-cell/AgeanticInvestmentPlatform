...
confirmed assumptions over a line the filer never tagged. There is nothing to map — the
fix is to make `cost_of_debt` suppliable by hand, exactly as the risk-free rate already
is, so that a filer's presentation choice cannot defeat a forecast the operator has
