"""What a book is worth, and what it cost — the arithmetic ADR 0079 refused to store.
...
* a **cost basis under the wrong convention** — plausible, right units, 33% out (ADR 0081);
...
agree on almost every book and disagree on the one in ADR 0081, which is why that example is
...
class TestTheConventionIsPooledAndNotFirstIn:
    """ADR 0081's worked example, which is where the three conventions disagree.
    ...
    def test_an_acquisition_cost_cannot_be_asked_for_a_disposal(self, context) -> None:
        ...
        with pytest.raises(CalculationError, match="ADR 0081"):
...
class TestEveryFigureIsARecordedCalculation:
    def test_each_one_carries_a_formula_and_its_inputs(self, context) -> None:
        """ADR 0079's claim, checked against the ledger rather than the prose.
